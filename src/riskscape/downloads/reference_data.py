...
ReferenceDataset = DirectZipDataset | CkanZipDataset
...
def expected_file_exists(dataset: ReferenceDataset) -> bool:
...
def download_direct_zip(dataset: DirectZipDataset, overwrite: bool) -> bool:
    ...
    print(
        f"[error] Downloaded {dataset.label}, "
        f"but {dataset.expected_file} was not found"
    )
    return False
...
def find_ckan_zip_url(dataset: CkanZipDataset) -> str | None:
    ...
    for package_id in dataset.package_ids:
        ...
        for resource in ranked:
            ...
            if (
                resource_url
                and score_resource(resource, dataset.resource_name_hints) >= 10
            ):
...
def selected_datasets(names: list[str] | None) -> list[ReferenceDataset]:
    """Resolve dataset group names to concrete datasets."""
    if not names:
        return [*NATURAL_EARTH_DATASETS, *SAERI_DATASETS]

    selected: list[ReferenceDataset] = []
    seen: set[str] = set()
    for name in names:
    ...
    for dataset in datasets:
        try:
            if isinstance(dataset, DirectZipDataset):
                success = download_direct_zip(dataset, overwrite=overwrite)
            else:
                success = download_ckan_zip(dataset, overwrite=overwrite)
