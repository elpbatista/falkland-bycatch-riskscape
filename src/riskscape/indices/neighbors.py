...
def build_neighbor_table() -> Path:
    """Build H3 neighbor table from uint64 grid."""
    ...
    out_file = out_dir / "h3_neighbors.parquet"
    neighbor_df.to_parquet(out_file, index=False)

    logger.info("Neighbor table saved: %s", out_file)
    logger.info("Total cells: %d", len(neighbor_df))
