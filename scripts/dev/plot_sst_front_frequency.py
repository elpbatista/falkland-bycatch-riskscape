"""
Compute and plot SST front frequency map (10-year).
"""
...
def main():
    ...
    tau = df["sst_grad"].quantile(0.75)
    print("Threshold τ =", tau)

    print("Computing front frequency")
    df["front_flag"] = (df["sst_grad"] > tau).astype("int8")

    freq = (
        df.groupby("h3", as_index=False)["front_flag"]
        .mean()
        .rename(columns={"front_flag": "front_frequency"})
    )
    ...
    grid.plot(
        column="front_frequency",
        ax=ax,
        legend=True
    )

    ax.set_title("SST Front Frequency (Top 25% Gradient)")
    ax.set_axis_off()
